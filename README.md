...
2026-09-23 김포(GMP)→제주(CJU, 17:00 이후)와 2026-09-27 귀국편을 성인 2명·어린이 2명으로 Google Flights에서 매시간 검색합니다. 두 구간의 검색 결과가 모두 반환될 때만 텔레그램으로 알립니다. 유료 API, 로그인, CAPTCHA 우회, 프록시, 탐지 회피는 사용하지 않습니다.

## 중요한 제한

- Google Flights는 좌석 재고 수를 명시하지 않습니다. 따라서 **4명을 승객으로 지정한 검색에서 출발·귀국 결과와 실제 총가격이 정상 반환됨**을 4인 예약 가능의 추정 기준으로 사용합니다. `Total price is unavailable`처럼 가격 없는 운항 스케줄은 매진 가능성이 있으므로 알림에서 제외합니다. 결제 직전 반드시 직접 확인하세요.
