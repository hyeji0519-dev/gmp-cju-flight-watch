...
2026-10-05와 2026-10-11 제주(CJU)→김포(GMP) **오후(12:00 이후) 출발** 편도를 성인 2명 기준으로 10분마다 확인합니다. 2석 이상 예약 가능한 항공편이 나타나면 텔레그램으로 알립니다. 감시 구간은 `src/config.js`의 `legs` 목록에서 바꿉니다. 유료 API, 로그인, CAPTCHA 우회, 프록시, 탐지 회피는 사용하지 않습니다.
...
- 첫 실행의 메시지 폭주를 막기 위해 화면 순서상 최대 20개 항공편을 추적합니다. 추적 대상이 바뀌면 새 항공편만 다시 알립니다.
...
- GitHub Actions 예약 실행은 정각을 보장하지 않으며 부하에 따라 수십 분 이상 늦거나 드물게 누락될 수 있습니다. cron은 10분 간격으로 설정했습니다.
- 무료 GitHub 호스팅 러너의 사용량·정책은 계정과 저장소 유형에 따라 달라질 수 있습니다.
- 각 날짜는 당일 12:00 KST가 지나면 감시에서 빠지고, 마지막 날짜(2026-10-11 12:00 KST) 이후에는 실행되어도 즉시 정상 종료합니다. 예약 workflow 자체를 삭제하지는 않습니다.
