...
2026-10-05 제주(CJU)→김포(GMP) **오후(12:00 이후) 출발** 편도를 성인 2명 기준으로 10분마다 확인합니다. 2석 이상 예약 가능한 항공편이 나타나면 텔레그램으로 알립니다. 감시 구간은 `src/config.js`의 `legs` 목록에서 바꿉니다. 유료 API, 로그인, CAPTCHA 우회, 프록시, 탐지 회피는 사용하지 않습니다.
...
- 2026-10-05 12:00 KST부터는 실행되어도 즉시 정상 종료합니다. 예약 workflow 자체를 삭제하지는 않습니다.
